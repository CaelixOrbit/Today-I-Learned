...
$$
X_1(\omega) = X(\omega) \sum_{n = -\infty}^{\infty} \delta(\omega - n \omega_当 $T_1 \geqslant 2t_m$ 或频域抽样间隔 $f_1$ 满足下列条件时：，频域抽样不会造成时域的重叠，即 $X(\omega)$ 完全可以由 $X_1(\omega)$ 得到重构。
x(t - n T_1)
$$
...
当 $T_1 \geqslant 2t_m$ 或频域抽样间隔 $f_1$ 足下列条件时，频域抽样不会造成时域的重叠，即 $X(\omega)$ 完全可以由 $X_1(\omega)$ 得到重构。
