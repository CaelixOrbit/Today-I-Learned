...
- **因果性**：假设 $h(t)$ 是一个因果系统的冲激响应，由因果性知 $h(t)$ 一定是单边信号（即当 $t < 0$ 时，$h(t) = 0$），因此在 $t \in (-\infty, \infty)$ 区间内一定有 
  ...
  $$
  H_{\mathrm{R}}(\omega) = \frac{1}{\pi} \int_{-\infty}^{\infty} \frac{H_{\mathrm{I}}(\lambda)}{\omega - \lambda} \, \mathrm{d} \lambda
  $$

  $$
  H_{\mathrm{I}}(\omega) = -\frac{1}{\pi} \int_{-\infty}^{\infty} \frac{H_{\mathrm{R}}(\lambda)}{\omega - \lambda} \, \mathrm{d} \lambda
  $$
  
  因此，对一个因果系统来说，$H(\omega)$ 的实部和虚部不是相互独立的，其中一个确定后另一个也随之确定。$H_{\mathrm{R}}(\omega)$ 和 $H_{\mathrm{I}}(\omega)$ 构成了希尔伯特变换对。
