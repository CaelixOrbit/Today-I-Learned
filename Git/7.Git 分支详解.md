...
Git 中的每个提交都是一个完整的文件系统快照，包含了文件的完整快照、提交时间、作者信息、提交说明以及指向父提交的指针。Git 默认基于这些内容计算生成唯一的 SHA-1 哈希值来标识每个提交。
...
HEAD 是 Git 中一个特殊的指针，通常指向当前所在的本地分支。可以将 HEAD 理解为 "当前正在工作的位置"。当 HEAD 指向某个分支时，该分支就被称为 "当前分支" 或 "活跃分支"。
